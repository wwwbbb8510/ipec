...
DEFAULT_PARTICLE_PATH = 'log/de_gbest.pkl'

def save_agent(agent, path=None):
    """
    save a agent using pickle

    :param agent: the agent to be saved
    :type agent: Agent
    :param path: path to write the serialised agent
    :type path: string
    """
    if path is None:
...
def load_agent(path=None):
    """
    load agent from persisted pickle file
...
class Agent(InterfaceArray):
    ...
    def __init__(self, id, length, layers=None, generation=0):
        ...
        :param id: agent ID
        :type id: int
        :param length: the length/dimension of the agent
        :type length: int
    ...
    def compare_with(self, agent):
        """
        compare this agent to the input agent

        :param agent: the input agent
        :type agent: Agent
        :return: 0: equal, 1: this agent is greater, -1: this agent is less
        :rtype: int
        """
        return self.compare_fitness(self.fitness, agent.fitness)

    def compare_fitness(self, fitness_1, fitness_2):
        """
        compare fitness of two agents @abstractmethod

        :param fitness_1: fitness of the first agent
        :type: tuple
        :param fitness_2: fitness of the second agent
        :type tuple
...
class CNNAgent(Agent):
    ...
    def __init__(self, id, length, max_fully_connected_length, layers, generation=0):
        ...
        :type generation: int
        """
        logging.info('===initialise CNN agent with ID: %d===', id)
        self.max_fully_connected_length = max_fully_connected_length
        super(CNNAgent, self).__init__(id, length, layers, generation)
    ...
    def fix_invalid_interface(self):
        ...
        logging.debug('===start fixing invalid layers of Agent-%d===', self.id)
        ...
        logging.debug('===finish fixing invalid layers of Agent-%d===', self.id)

    def initialise(self):
        """
        initialise a agent with random settings
        """
        logging.info('===start initialising CNN agent===')
        for i in range(self.length):
        ...
        logging.info('===finish initialising CNN agent===')
        return self
