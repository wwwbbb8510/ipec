...
First, a novel encoding strategy inspired by computer networks which empowers particle vectors to easily encode CNN layers is proposed; Second, in order to allow the proposed method to learn variable-length CNN architectures, a disabled layer is designed to hide some dimensions of the particle vector to achieve variable-length particles; Third, since the learning process on large data is slow, pick partial dataset will be randomly picked for the evaluation to dramatically speed it up. The proposed algorithm is examined and compared with 12 existing algorithms including the state-of-art methods on 3 widely used image classification benchmark datasets. The experimental results prove that the proposed algorithm is a strong competitor to the state-of-art algorithms in terms of classification error. Other than that, this is the first work using PSO for evolving the architectures of CNNs.
\end{abstract}
...
\begin{enumerate}
	%to be done: 1 and 2 need to rewrite. discuss%
...
% to be done: why this can be used to develop? motivation
...
\begin{algorithm}
	...
	\end{algorithmic}
\end{algorithm}
% to be done: add an update equation and explain
% to be done: what's the major difference from standard PSO
